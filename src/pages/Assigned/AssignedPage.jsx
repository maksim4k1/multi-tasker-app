...
import getAssignedAction from "../../redux/actions/assigned/getAssignedAction";
...
import getFilteredAssignedAction from "../../redux/actions/assigned/getFilteredAssignedAction";
...
function AssignedPage ({state, tasks, countOfTasks, countOfCompletedTasks, user, getAssigned, getFilteredAssigned}) {
  ...
  return(
    <Main>
      <Header lLink="/" title="Назначенные" rIcon={<FilterIcon/>} rOnClick={() => modalHandler(true)} />
      ...
      <Info>{countOfTasks || 0} задач / {countOfCompletedTasks || 0} выполнено</Info>
      ...
      <NavBar active={ActivePage.tasks} />
    </Main>
  );
}
...
const mapStateToProps = (state) => ({
  ...
  tasks: state.assigned.tasks,
  countOfTasks: state.assigned.countOfTasks,
  countOfCompletedTasks: state.assigned.countOfCompletedTasks,
  user: state.auth.profile,
});
const mapDispatchToProps = {
  getAssigned: getAssignedAction,
  getFilteredAssigned: getFilteredAssignedAction,
};
